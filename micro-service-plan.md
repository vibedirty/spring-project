...
P1 已在 `feature/ms-p1-gateway` 实现并完成技术链路验收：
...
- 用户端和管理端的本地开发代理默认统一指向 `http://localhost:9000`；
...
> 完成状态（2026-08-28）：已实现。账户内部 DTO 接口、订单域 Feign 迁移、请求头透传、显式超时、客户端负载均衡、Sentinel 限流/熔断、Nacos 规则持久化和本地故障实验入口均已落地。创建订单在开启本地事务前取得地址与用户快照，历史订单查询仍只读取订单快照。
