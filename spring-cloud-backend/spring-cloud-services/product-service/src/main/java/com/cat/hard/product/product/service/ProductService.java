...
import com.cat.hard.product.product.enums.ProductSort;
...
public class ProductService {
	...
	@Transactional
	public void increaseSales(ProductSalesUpdateRequest request) {
		if (request == null || request.getItems() == null || request.getItems().isEmpty()) {
			return;
		}
		for (ProductSalesUpdateRequest.SalesItem item : request.getItems()) {
			...
			if (rows == 0) {
				log.warn("更新商品销量失败：orderNo={}, productId={}, quantity={}",
						request.getOrderNo(), item.getProductId(), item.getQuantity());
			}
		}
	}
	...
	public List<ProductSummary> getBatchSummaries(List<Long> ids) {
	...
	public List<ProductQuoteDto> getBatchQuotes(List<Long> ids) {
		if (ids == null || ids.isEmpty()) {
			return java.util.Collections.emptyList();
		}
		List<Product> products = productMapper.selectByIds(ids);
